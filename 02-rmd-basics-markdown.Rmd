...
```{r}
library(reticulate)
use_python('/home/rifqoi/.conda/envs/data-exercises/bin/python3')
use_condaenv('data-exercises')
```

```{python python-code, echo=FALSE, fig.cap="Python code"}
import pandas as pd
df1 = pd.DataFrame(
    {
        "A": ["A0", "A1", "A2", "A3"],
...
```
```{r python-table}
knitr::kable(py$df1, caption = "Python Table")
```
